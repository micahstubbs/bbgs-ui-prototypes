...
function drawGraph(graph) {
  ...
  // setup the background drag
  canvas
    .call(
      d3
        .drag()
        .subject(backgroundDragSubject)
        .on('drag', backgroundDragged)
    )
    .call(
      d3
        .zoom()
        .scaleExtent([1 / 2, 8])
        .on('zoom', backgroundZoomed)
    );
  ...
  d3
    ...
    .call(
      d3
        .drag()
        .container(canvas)
        .subject(clickDragSubject)
        .on('start', clickDragStarted)
        .on('drag', clickDragged)
        .on('end', clickDragEnded)
    );
...
function clickDragStarted() {
  if (!d3.event.active) simulation.alphaTarget(0.3).restart();
  d3.event.subject.fx = d3.event.subject.x;
  d3.event.subject.fy = d3.event.subject.y;
}

function clickDragged() {
  d3.event.subject.fx = d3.event.x;
  d3.event.subject.fy = d3.event.y;
}

function clickDragEnded() {
...
function backgroundDragSubject() {
  var i,
