...
(function(global, factory) {
  ...
  function rectCollide(bbox) {
    var nodes, boundingBoxes, strength = 1, iterations = 1;

    if (typeof bbox !== 'function')
      bbox = constant(bbox == null ? [[0, 0][(1, 1)]] : bbox);

    function force() {
      var i, n = nodes.length, tree, node, xi, yi, bbi, nx1, ny1, nx2, ny2;
      ...
      for (var k = 0; k < iterations; ++k) {
        ...
        for (i = 0; i < n; ++i) {
          var node = nodes[i];
          (bbi = boundingBoxes[i]), (xi = node.x + node.vx);
          yi = node.y + node.vy;
          nx1 = xi + bbi[0][0];
          ny1 = yi + bbi[0][1];
          nx2 = xi + bbi[1][0];
    ...
    force.strength = function(_) {
      return arguments.length
        ? ((strength = typeof _ === 'function'
            ? _
            : constant(+_)), initializeStrength(), force)
        : strength;
    };

    force.distance = function(_) {
      return arguments.length
        ? ((distance = typeof _ === 'function'
            ? _
            : constant(+_)), initializeDistance(), force)
        : distance;
    };
  ...
  function simulation(nodes) {
    ...
    return (simulation = {
      ...
      nodes: function(_) {
        return arguments.length
          ? ((nodes = _), initializeNodes(), forces.each(
              initializeForce
            ), simulation)
          : nodes;
      },
  ...
  function manyBody() {
    ...
    function force(_) {
      var i,
        n = nodes.length,
        tree = d3Quadtree.quadtree(nodes, x$2, y$2).visitAfter(accumulate);
      for (
        (alpha = _), (i = 0);
        i < n;
        ++i
